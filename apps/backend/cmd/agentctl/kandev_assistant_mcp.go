...
var assistantBrokerTools = []assistantBrokerTool{
	...
	{"create_task", "Delegate a task when the owner selected design or execute. request requires operation_id, expected_intent_revision, objective_id, execution_mode, workflow_id, assignee, title (60 characters or fewer) and description. Respect the objective's scope.", http.MethodPost, "/runtime/tasks"},
	{"manage_task", "Manage task id through native actions: edit (title, description, priority, parent_id), move (workflow_step_id, optional workflow_id and position), assign (assignee), adopt, start, stop, message (prompt, optional session_id), repair_session (optional session_id; recovers a session stopped on a provider login failure), archive or delete. request requires operation_id, expected_intent_revision, objective_id, action and current scoped context. Unknown outcomes must never be blindly retried.", http.MethodPost, "/runtime/tasks/:id/manage"},
