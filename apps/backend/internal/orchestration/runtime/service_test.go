...
func newRuntime(t *testing.T) (*Service, *sqlx.DB, string) {
	...
	require.NoError(t, err)
	svc := &Service{Enabled: true, Repo: repo, Personas: &personas.Service{Profiles: profiles, Repo: repo}, Runs: runs, Queue: runservice.New(runs, nil, log, nil), Auth: runtimeauth.NewAgentAuth(""), Tasks: &testTasks{tasks: map[string]*taskmodels.Task{}}, APIURL: "http://localhost:1"}
