...
// memoryColumns names every column read into models.AgentMemory, so columns
// retained on older databases never break the scan.
const memoryColumns = `id, agent_profile_id, layer, key, content, metadata, created_at, updated_at`
...
func (r *Repository) ListAgentMemory(ctx context.Context, agentInstanceID string) ([]*models.AgentMemory, error) {
	...
	err := r.ro.SelectContext(ctx, &entries, r.ro.Rebind(
		`SELECT `+memoryColumns+` FROM orchestration_memory WHERE agent_profile_id = ? ORDER BY layer, key`),
		agentInstanceID)
	if err != nil {
		return nil, err
	}
...
func (r *Repository) GetAgentMemory(ctx context.Context, agentInstanceID, layer, key string) (*models.AgentMemory, error) {
	...
	err := r.ro.QueryRowxContext(ctx, r.ro.Rebind(
		`SELECT `+memoryColumns+` FROM orchestration_memory WHERE agent_profile_id = ? AND layer = ? AND key = ?`),
