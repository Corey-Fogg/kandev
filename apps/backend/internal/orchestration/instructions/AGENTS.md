...
- Write: `create_task`, `manage_task`, `task_status`, `manage_workspace`, `remember`, `forget`, `comment`. `manage_task` and `task_status` accept `ids` to apply one change to several tasks.
